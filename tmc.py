...
# --- 1. CẤU HÌNH & CSS ELITE (FIX LỖI CHỒNG CHÉO) ---
...
st.markdown("""
    ...
    /* Thiết kế Card Trang chủ chuyên nghiệp */
    .home-card {
    ...
    .home-content h3 { color: #00263e; font-size: 22px; margin-bottom: 15px; }
    ...
    .info-label { font-size: 11px; color: #94a3b8; font-weight: 700; text-transform: uppercase; margin-bottom: 2px; }
    .info-value { font-size: 14px; color: #1e293b; font-weight: 500; margin-bottom: 10px; }
    
    /* Nút hành động không đè bóng */
    .action-link {
        color: #0369a1; text-decoration: none; font-weight: 700;
        padding: 8px 14px; border-radius: 8px; background: #f1f5f9;
        font-size: 12px; display: inline-block; margin: 4px;
    }
...
if id_khach:
    ...
    if row:
        ...
        new_n = f"<div style='color:red;'>{t_now} 🔥 KHÁCH ĐANG XEM MINH HOẠ PDF</div>" + str(row['note'])
        conn.execute("UPDATE leads SET note=?, last_updated=? WHERE cell=?", (new_n, datetime.now(NY_TZ).isoformat(), id_khach))
        conn.commit()
        st.markdown(f"<h2 style='text-align:center; color:#00263e;'>Hồ sơ bảo hiểm: {row['name']}</h2>", unsafe_allow_html=True)
        if row['pdf_file']:
        ...
        else: st.warning("Hồ sơ đang được chuẩn bị. Vui lòng quay lại sau.")
    conn.close(); st.stop()
...
# --- TRANG CHỦ (BỐ CỤC ELITE) ---
if selected == "Trang Chủ":
    st.markdown(f"<h1 style='text-align:center; color:#00263e; padding: 30px 0;'>{prof.get('slogan')}</h1>", unsafe_allow_html=True)
    c1, c2 = st.columns(2, gap="large")
    
    with c1:
        img_n = img_to_b64(prof.get('img_national'))
        st.markdown(f"""
            <div class='home-card'>
                <div class='home-img-container'>
                    <img src="data:image/jpeg;base64,{img_n}" onerror="this.src='https://via.placeholder.com/600x400?text=National+Life'">
                </div>
                <div class='home-content'>
                    <h3>National Life Group</h3>
                    <p>Phục vụ khách hàng từ năm 1848. Chúng tôi cam kết mang lại giá trị bền vững và sự an tâm tài chính tuyệt đối cho các gia đình trên khắp Hoa Kỳ.</p>
                </div>
            </div>""", unsafe_allow_html=True)

    with c2:
        img_i = img_to_b64(prof.get('img_iul'))
        st.markdown(f"""
            <div class='home-card'>
                <div class='home-img-container'>
                    <img src="data:image/jpeg;base64,{img_i}" onerror="this.src='https://via.placeholder.com/600x400?text=IUL+Solution'">
                </div>
                <div class='home-content'>
                    <h3>Giải pháp IUL Elite</h3>
                    <p>{prof.get('slogan')}. Lựa chọn thông minh để bảo vệ thu nhập, tích lũy tài sản và chuẩn bị cho một tương lai hưu trí an nhàn.</p>
                </div>
            </div>""", unsafe_allow_html=True)
...
# --- VẬN HÀNH (FULL 10 TRƯỜNG & PDF) ---
            ...
            with st.container():
                ...
                <div class="client-card">
                    <div style="display: flex; gap: 20px;">
                        <div style="flex: 2.5;">
                            <div style="font-size: 20px; font-weight: 800; color: #00263e; margin-bottom: 12px;">{row['name']}</div>
                        ...
                        <div style="flex: 3; background: #f8fafc; border-radius: 8px; padding: 12px; height: 180px; overflow-y: auto;">
                            <div class="info-label">Nhật ký hành vi</div>
                            <div style="font-size: 13px;">{row['note']}</div>
                        </div>
                    </div>
                ...
                with c_n:
                    with st.form(key=f"nt_{row['id']}", clear_on_submit=True):
                        ...
                        if st.form_submit_button("LƯU", use_container_width=True):
                            t_s = datetime.now(NY_TZ).strftime('[%m/%d %H:%M]'); n_n = f"<div>{t_s} {ni}</div>" + str(row['note'])
                ...
                with c_up:
                    with st.popover("📁 UP PDF MINH HỌA", use_container_width=True):
                        f = st.file_uploader("Chọn PDF", type="pdf", key=f"f_{row['id']}")
                        if st.button("XÁC NHẬN LƯU", key=f"s_{row['id']}"):
                            if f: conn.execute("UPDATE leads SET pdf_file=? WHERE id=?", (f.read(), row['id'])); conn.commit(); st.success("Đã lưu!"); st.rerun()
                with c_s:
    ...
    with t2:
        st.markdown("### ➕ THIẾT LẬP HỒ SƠ MỚI (ĐỦ 10 TRƯỜNG)")
        with st.form("add_full"):
            r1 = st.columns(3); n = r1[0].text_input("Tên khách hàng"); i = r1[1].text_input("Mã CRM ID"); l = r1[2].text_input("Link CRM")
            r2 = st.columns(3); c = r2[0].text_input("Số Cell"); w = r2[1].text_input("Số Work"); e = r2[2].text_input("Email")
            r3 = st.columns(3); s = r3[0].text_input("State (Bang)"); o = r3[1].text_input("Owner (Người phụ trách)", value="Cong"); t = r3[2].text_input("Tags")
            st_ = st.selectbox("Status", ["New", "Contacted", "Following", "Closed"])
            if st.form_submit_button("LƯU HỒ SƠ HỆ THỐNG", use_container_width=True):
...
# --- MẮT THẦN ---
    ...
    for _, row in df.iterrows():
        with st.container():
            st.markdown(f"""
            <div style="background:white; padding:20px; border-radius:12px; border-left:6px solid #ef4444; margin-bottom:15px; box-shadow: 0 4px 6px rgba(0,0,0,0.05);">
                <div style="display:flex; justify-content:space-between;">
                    <div style="font-size:18px; font-weight:700; color:#00263e;">{row['name']}</div>
                    <div class="eye-active">🔥 ĐANG TRUY CẬP</div>
                </div>
                <div style="margin-top:10px; font-size:14px; color:#64748b;">Số điện thoại: <b>{row['cell']}</b> | Người phụ trách: <b>{row['owner']}</b></div>
                <div style="margin-top:10px; background:#fef2f2; padding:15px; border-radius:8px; font-size:14px; border: 1px dashed #fecaca;">{row['note']}</div>
            </div>""", unsafe_allow_html=True)
    conn.close()

# --- CẤU HÌNH (FIX LỖI UPLOAD) ---
elif selected == "Cấu Hình":
    st.markdown("## ⚙️ Cấu Hình Hệ Thống Elite")
    with st.form("config_full"):
        new_sl = st.text_input("Slogan thương hiệu", value=prof.get('slogan'))
        c1, c2, c3 = st.columns(3)
        up_l = c1.file_uploader("Logo Sidebar", type=['png', 'jpg', 'jpeg'])
        up_n = c2.file_uploader("Ảnh National Life", type=['png', 'jpg', 'jpeg'])
        up_i = c3.file_uploader("Ảnh IUL Solution", type=['png', 'jpg', 'jpeg'])
        ...
        if st.form_submit_button("LƯU TẤT CẢ THAY ĐỔI"):
            ...
            conn.commit(); conn.close()
            st.cache_data.clear() # Quan trọng để hình hiện ngay
            st.success("Cấu hình đã được cập nhật!"); st.rerun()
