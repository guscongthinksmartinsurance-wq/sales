...
def init_db():
    ...
    c.execute('''CREATE TABLE IF NOT EXISTS leads (id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT)''')
    
    # DANH SÁCH 11 CỘT CẦN CÓ (Fix lỗi OperationalError)
    full_cols = [
        ('name', 'TEXT'), ('crm_id', 'TEXT'), ('crm_link', 'TEXT'),
        ('cell', 'TEXT'), ('work', 'TEXT'), ('email', 'TEXT'),
        ('state', 'TEXT'), ('owner', 'TEXT'), ('tags', 'TEXT'),
        ('status', 'TEXT'), ('last_updated', 'TEXT'), ('note', 'TEXT')
    ]
    
    for col_name, col_type in full_cols:
        try:
...
if selected == "Trang Chủ":
    ...
    col_left, col_right = st.columns(2, gap="large") # gap="large" để không bị dính lẹo
    with col_left:
    ...
    with t1:
        st.markdown(f"**Tổng cộng:** {len(df_m)} hồ sơ")
        ...
        for idx, row in filtered.iterrows():
            with st.container(border=True):
                ...
                with ci:
                    ...
                    st.write(f"Cell: {row['cell']} | Owner: {row['owner']}")
                with ce:
                    with st.popover("SỬA"):
